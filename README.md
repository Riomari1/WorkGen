...
- In-memory workout response cache with TTL and regenerate bypass
...
- The current cache is process-local, which is useful for a fast MVP but not shared across Vercel instances.
...
- Phase 2 should move the cache to Redis or Upstash so results survive cold starts and can be reused across Vercel regions/instances.
